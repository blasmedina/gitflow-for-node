...
commit() {
  ...
  if [ -z "$MESSAGE" ]; then
    ...
    git add --all && git commit --gpg-sign --message "$MESSAGE"
  fi
}
...
finish_release() {
  ...
  local CURRENT_BRANCH=$(get_current_branch) && validate_branch $CURRENT_BRANCH && ask_version && merge $CURRENT_BRANCH $MASTER_BRANCH && update_pkg_version $VERSION
...
finish_hotfix() {
  ...
  local CURRENT_BRANCH=$(get_current_branch) && validate_branch $CURRENT_BRANCH && merge $CURRENT_BRANCH $MASTER_BRANCH && update_pkg_version $PATCH_VERSION
  local CURRENT_VERSION=$(get_current_version) && fast_commit && create_tag $CURRENT_VERSION
  local CURRENT_TAG=$(get_current_tag) && merge $CURRENT_TAG $DEVELOP_BRANCH && fast_commit
  publish_branch $MASTER_BRANCH
  publish_branch $DEVELOP_BRANCH
